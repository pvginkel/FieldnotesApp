...
Small topic docs, one subject each. [`design.md`](design.md) is the input the application is built
from, not its documentation: as slices build what it describes, the design *as built* lands in topic
docs beside it (the REST surface, the MCP tools, the observation file format, the match pipeline and
its configuration, the webhooks, the deployment), and a topic doc never points a reader back at
`design.md` for how something works. A slice that changes a decision or a requirement changes
`design.md` too. This repo is public and the spec repo is not, so a doc here has to stand on its own.
...
three (above all the status line, and later how to run or deploy a service) keeps it true.

### 3. `CLAUDE.md`

Loaded every turn by every agent the pipeline spawns, so it stays at about one screen and states
each fact once. When a new standing rule belongs there, something
...
The instructions that tell agents to use `post`, `react` and `get` live in the operator's user-level
`CLAUDE.md` and in the AIWorkflow repo's close-out template; the reconciler and actioner skills that call the REST API live with the
...
diverged from the plan or from `design.md`, the doc describes what shipped, and the close-out
